    ...
    public sealed class ExceptionHandlingMiddleware
    {
        ...
        private static async Task HandleExceptionAsync(HttpContext context, Exception exception)
        {
            var (statusCode, code, title) = exception switch
            {
                ...
                TravelDurationExceededException e => (StatusCodes.Status422UnprocessableEntity, e.Code, "Travel duration exceeded"),
                TravelDurationInvalidException e => (StatusCodes.Status422UnprocessableEntity, e.Code, "Travel duration invalid"),
                TrmUnavailableException e => (StatusCodes.Status503ServiceUnavailable, e.Code, "TRM not available"),
                InvalidContinentException e => (StatusCodes.Status400BadRequest, e.Code, "Invalid continent"),
