...
public sealed class AppealClaimHandlerTests
{
    private readonly IClaimRepository _claimRepository = Substitute.For<IClaimRepository>();
    private readonly IUnitOfWork _unitOfWork = Substitute.For<IUnitOfWork>();
    private AppealClaimHandler Sut => new(_claimRepository, _unitOfWork);
    ...
    private static DomainClaim BuildRejectedClaim()
    {
        ...
        claim.Reject("Not covered", "investigator@test.com");
        return claim;
    }

    [Fact]
    public async Task Handle_WhenClaimIsRejected_ShouldAppealAndReturnResponse()
    {
        // GIVEN
        var claim = BuildRejectedClaim();
        _claimRepository.GetByIdAsync(claim.Id, Arg.Any<CancellationToken>()).Returns(claim);

        // WHEN
        var result = await Sut.Handle(
            new AppealClaimCommand { ClaimId = claim.Id, ResponsibleUser = "user@test.com", Observations = "New evidence" },
            CancellationToken.None);

        // THEN
        result.Should().NotBeNull();
        result.Status.Should().Be(nameof(ClaimStatus.Appealed));
    }

    [Fact]
    public async Task Handle_WhenClaimIsRejected_ShouldCallUpdateAndSave()
    {
        // GIVEN
        var claim = BuildRejectedClaim();
        _claimRepository.GetByIdAsync(claim.Id, Arg.Any<CancellationToken>()).Returns(claim);
        ...
        // WHEN
        await Sut.Handle(
            new AppealClaimCommand { ClaimId = claim.Id, ResponsibleUser = "user@test.com" },
            CancellationToken.None);
    ...
    public async Task Handle_WhenClaimNotFound_ShouldThrowNotFoundException()
    {
        ...
        var act = async () => await Sut.Handle(
            new AppealClaimCommand { ClaimId = Guid.NewGuid(), ResponsibleUser = "user@test.com" },
            CancellationToken.None);
    ...
    [Fact]
    public async Task Handle_WhenClaimNotRejected_ShouldThrowInvalidStateException()
    {
        // GIVEN
        var policy = BuildActivePolicy();
        var today = DateOnly.FromDateTime(DateTime.UtcNow);
