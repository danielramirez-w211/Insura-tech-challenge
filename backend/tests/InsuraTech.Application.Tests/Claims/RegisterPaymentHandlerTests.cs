...
public sealed class RegisterPaymentHandlerTests
{
    ...
    private RegisterPaymentHandler Sut => new(_claimRepository, _unitOfWork);
    ...
    private static DomainClaim BuildApprovedClaim()
    {
        ...
        claim.Approve(800m, "approver@test.com");
        return claim;
    }

    [Fact]
    public async Task Handle_WhenClaimIsApproved_ShouldRegisterPaymentAndReturnResponse()
    {
        // GIVEN
        var claim = BuildApprovedClaim();
        _claimRepository.GetByIdAsync(claim.Id, Arg.Any<CancellationToken>()).Returns(claim);

        // WHEN
        var result = await Sut.Handle(
            new RegisterPaymentCommand { ClaimId = claim.Id, ResponsibleUser = "finance@test.com" },
            CancellationToken.None);

        // THEN
        result.Should().NotBeNull();
        result.Status.Should().Be(nameof(ClaimStatus.Paid));
    }

    [Fact]
    public async Task Handle_WhenClaimIsApproved_ShouldCallUpdateAndSave()
    {
        // GIVEN
        var claim = BuildApprovedClaim();
        _claimRepository.GetByIdAsync(claim.Id, Arg.Any<CancellationToken>()).Returns(claim);
    ...
    public async Task Handle_WhenClaimNotFound_ShouldThrowNotFoundException()
    {
        ...
        var act = async () => await Sut.Handle(
            new RegisterPaymentCommand { ClaimId = Guid.NewGuid(), ResponsibleUser = "finance@test.com" },
            CancellationToken.None);
    ...
    [Fact]
    public async Task Handle_WhenClaimNotApproved_ShouldThrowInvalidStateException()
    {
        // GIVEN
        var policy = BuildActivePolicy();
        var today = DateOnly.FromDateTime(DateTime.UtcNow);
